          ...
          <PopoverContent className="email__snooze__content">
            <div className="email__snooze__content__left">
              ...
              <div className="grid min-w-[250px] gap-1">
                <Button variant="ghost" onClick={() => setDate(laterThisDay)}>
                  Later today
                  <span>{format(laterThisDay, 'E, h:m b')}</span>
                </Button>
                <Button variant="ghost" onClick={() => setDate(tomorrow)}>
                  Tomorrow
                  <span>{format(tomorrow, 'E, h:m b')}</span>
                </Button>
                <Button variant="ghost" onClick={() => setDate(thisWeekend)}>
                  This weekend
                  <span>{format(thisWeekend, 'E, h:m b')}</span>
                </Button>
                <Button variant="ghost" onClick={() => setDate(nextWeek)}>
                  Next week
